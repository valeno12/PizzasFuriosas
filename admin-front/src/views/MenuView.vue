...
const createAsPromotion = ref(false)
...
const categoryTabs = computed(() => [
  ...
  ...[...categories.value]
    .sort(
      (a, b) =>
        Number(b.name.toLowerCase() === 'promos') - Number(a.name.toLowerCase() === 'promos'),
    )
...
const catalogGroups = computed(() =>
  [
    {
      title: 'Promos',
      description: 'Combos con precio propio y sus productos incluidos.',
      promo: true,
      items: items.value.filter((item) => item.components?.length),
    },
    {
      title: 'Productos',
      description: 'Productos individuales de la carta.',
      promo: false,
      items: items.value.filter((item) => !item.components?.length),
    },
...
function openCreate(promotion = false) {
  createAsPromotion.value = promotion
...
<template>
  <div class="mx-auto w-full max-w-[1180px]">
    ...
    <div class="grid min-w-0 grid-cols-1 gap-4 px-4 pb-8 md:px-8 md:pb-10">
      <!-- Stats -->
      <div class="grid grid-cols-3 gap-2">
        <StatCard
          class="min-w-0 !p-2 [&_.material-symbols-outlined]:hidden [&>div:first-child]:text-[0.6rem] sm:!p-3 sm:[&_.material-symbols-outlined]:inline sm:[&>div:first-child]:text-[0.68rem]"
          label="Disponibles"
          icon="check_circle"
      ...
      <section
        ...
        class="grid grid-cols-1 gap-4 pb-4 min-[360px]:grid-cols-2 md:grid-cols-3 xl:grid-cols-4"
      >
      ...
      >
        <template v-for="group in catalogGroups" :key="group.title">
          <div class="col-span-full flex items-center gap-3 pt-2">
            <span
              v-if="group.promo"
              class="material-symbols-outlined rounded-xl bg-primary-soft p-2 text-primary"
              aria-hidden="true"
              >local_offer</span
            >
            <div>
              <h2 class="text-lg font-extrabold">
                {{ group.title }}
                <span class="ml-1 text-sm font-semibold text-muted">{{ group.items.length }}</span>
              </h2>
          ...
          <article
            ...
            :key="item.id"
            :class="[
              'flex min-w-0 flex-col overflow-hidden rounded-2xl border bg-surface',
              group.promo ? 'border-primary' : 'border-line',
            ]"
          >
            <div class="relative aspect-square overflow-hidden bg-surface-2">
              ...
              <span
                v-if="group.promo"
                class="absolute left-2 top-2 rounded-md bg-primary px-2 py-1 text-[0.65rem] font-black tracking-wider text-primary-foreground"
                >PROMO</span
              >
            ...
            <button
              type="button"
              class="flex flex-1 flex-col gap-2 p-3 text-left transition-colors hover:bg-surface-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary"
              :aria-label="`Editar ${item.name}`"
              @click="openEdit(item)"
            >
              <strong class="block break-words text-[0.94rem] leading-snug">{{ item.name }}</strong>
              <span v-if="group.promo" class="text-xs leading-relaxed text-muted">{{
                item.components.map((c) => `${c.quantity}× ${c.productName}`).join(' · ')
              }}</span>
              <small v-else class="text-xs text-muted">{{ item.category }}</small>
              <span
                v-if="item.freeDelivery"
                class="inline-flex items-center gap-1 rounded-md bg-primary-soft px-2 py-1 text-[0.65rem] font-bold text-primary"
              >
              ...
              <span
                class="mt-auto flex w-full flex-wrap items-center justify-between gap-2 border-t border-line-soft pt-2"
              >
                <span class="font-black text-primary">{{ formatMoney(item.price) }}</span>
                <span class="inline-flex items-center gap-1 text-xs font-semibold text-muted">
    ...
    <ProductEditorDrawer
      ...
      :create-as-promotion="createAsPromotion"
